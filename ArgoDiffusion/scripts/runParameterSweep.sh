...
#     if [ "$output_file_size" -gt "86" ]; then
        echo "{" \"area\": $area, \"time_coverage\": $time_coverage, \"num_of_params\": $num_of_params, \"dataset_size\": $dataset_size, \"output_file_size\": $output_file_size, \"execution_time\": $execution_time,\"execution_date\": \"$EXECUTION_DATE\" , \"configuration\": $conf, \"num_of_nodes\":$NUMBER_OF_NODES, \"num_of_tasks\":$NUMBER_OF_TASKS, \"executing_node\":\"$MY_IP\""}"
#     fi
}
...
#                         python partitioning.py configuration_new.json $NUMBER_OF_TASKS
#                         send_messages
...
NUMBER_OF_TASKS=$((NUMBER_OF_NODES*8))
